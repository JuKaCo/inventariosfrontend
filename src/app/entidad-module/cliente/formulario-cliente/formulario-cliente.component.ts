...
export class FormularioClienteComponent implements OnInit {
  ...
  guardarDatos(): void {
    ...
    this.entidadService.setCliente(data).subscribe(response => {
      if (response.success) {
  ...
  modificarDatos(): void {
    ...
    this.entidadService.setEditaCliente(valores, data.id).subscribe(response => {
      if (response.success) {
        this.respform.emit({ tipo: this.tipo, success: true, message: response.message });
        this.messageService.add({ severity: 'success', summary: this.modulo, detail: response.message });
        this.displayFrm = false;
  ...
  editar(id: string): void {
    ...
    this.displayHeader = 'Formulario ' + this.header;
    //this.formulario.get('codigo')?.disable();
    this.entidadService.getRegistroCliente(id).subscribe(response => {
      if (response.success) {
  ...
  eliminar(event: any, id: string) {
    ...
    setTimeout(() => {
      this.confirmationService.confirm({
        ...
        accept: () => {
          //confirm action
          this.entidadService.setEliminaCliente(id).subscribe(response => {
            if (response.success) {
              this.respform.emit({ tipo: this.tipo, success: true, message: response.message });
