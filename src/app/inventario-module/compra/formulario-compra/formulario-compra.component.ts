...
export class FormularioCompraComponent implements OnInit {
  ...
  textCambiarEstado: string = "¿Esta seguro de modificar el estado?.";
  ...
  initForm(): void {
    ...
    this.formulario.addControl('gestion', new FormControl({ value: '', disabled: false }, [ValidacionService.numberValidator]));
    this.formulario.addControl('descripcion', new FormControl({ value: '', disabled: false }, []));
  ...
  modificarEstado(event: any, id: any, estado: any) {
    ...
    setTimeout(() => {
      this.confirmationService.confirm({
        target: event.target,
        message: this.textCambiarEstado,
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
          //confirm action
